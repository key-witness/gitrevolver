...
func printSetupGuide(out io.Writer, identity, project string) {
	...
	fmt.Fprintln(out)
	fmt.Fprintln(out, "Install get-secret (optional, for non-provider API keys in /etc/secrets/keys.env):")
	fmt.Fprintln(out, "  # 1. Create the privileged reader — run in your own terminal:")
	fmt.Fprintln(out, "  cat << 'EOF' | sudo tee /usr/local/bin/get-secret-privileged > /dev/null")
	fmt.Fprintln(out, "  #!/bin/bash")
	fmt.Fprintln(out, "  awk -F= -v k=\"$1\" '$1==k{print substr($0,length($1)+2);exit}' /etc/secrets/keys.env")
	fmt.Fprintln(out, "  EOF")
	fmt.Fprintln(out, "  sudo chmod 755 /usr/local/bin/get-secret-privileged")
	fmt.Fprintln(out, "  # 2. Allow passwordless sudo for that script only:")
	fmt.Fprintln(out, "  echo \"$(whoami) ALL=(root) NOPASSWD: /usr/local/bin/get-secret-privileged\" | sudo tee /etc/sudoers.d/get-secret > /dev/null")
	fmt.Fprintln(out, "  sudo chmod 440 /etc/sudoers.d/get-secret")
	fmt.Fprintln(out, "  # 3. Verify:")
	fmt.Fprintln(out, "  get-secret <alias>   # should return value non-interactively")
	fmt.Fprintln(out)
